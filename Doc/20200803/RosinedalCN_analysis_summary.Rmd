...
data.frame(scores(pyr_litter_rda_f, display = "species", choices = 1, scaling = 3)) %>%
...
data.frame(scores(pyr_humus_rda_f, display = "species", choices = 1, scaling = 3)) %>%
  mutate(pyr_comp = row.names(.),
         RDA1 = round(RDA1, 3)) %>%
  select(pyr_comp, RDA1) %>%
  arrange(RDA1) %>%
...
data.frame(scores(nmr_litter_rda_f, display = "species", choices = 1, scaling = 3)) %>% 
...
data.frame(scores(nmr_humus_rda_f, display = "species", choices = 1, scaling = 3)) %>% 
  mutate(nmr_comp = row.names(.),
         RDA1 = round(RDA1, 3)) %>% 
  select(nmr_comp, RDA1) %>% 
  arrange(RDA1) %>% 
...
dlply(pyr_all_raw, .(horizon), function(x){
  cor.test(x$leaf_d15N, x$`N-compound`, method = "spearman")
})
```
...
dlply(pyr_all_raw, .(horizon), function(x){
  cor.test(x$CNratio, x$`N-compound`, method = "spearman")
})
```
